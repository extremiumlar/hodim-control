...
def _verify_secret(request: Request) -> None:
    if not settings.crm_webhook_secret:
        # Sekret sozlanmagan — endpoint yopiq turadi (himoyasiz qabul qilmaymiz)
        raise HTTPException(
            status.HTTP_403_FORBIDDEN, "CRM_WEBHOOK_SECRET .env'da sozlanmagan — endpoint o'chiq"
        )
    ...
    # header NOMLARI (qiymatlarsiz — sekret log'ga tushmasin) ro'yxatidan
    # kalit qaysi kanalda kelayotgani ko'rinadi, keyin _SECRET_HEADERS'ga
    # o'sha nom qo'shiladi.
    logger.warning(
